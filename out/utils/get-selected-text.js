...
const getSelectedText = (editor) => {
    ...
    for (let index = text.length - offset_l; index > 0; index--) {
        ...
        if (textOff !== "." && re.test(textOff)) {
            ...
            if (/[^A-Z]/.test(text.charAt(offset_l))) {
                return new vscode_1.Selection(editor.document.positionAt(0), editor.document.positionAt(0));
            }
    ...
    return new vscode_1.Selection(editor.document.positionAt(offset_l), editor.document.positionAt(offset_r));
};
